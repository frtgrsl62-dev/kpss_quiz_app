...
def konu_secim_page(ders):
    ...
    if ders_notu_link:
        st.markdown(
            f"<a href='{ders_notu_link}' target='_blank'><button style='background-color: ; color: ; padding:6px; border:none; border-radius:10px; cursor:pointer;'>📚 Ders Notları</button></a>",
            unsafe_allow_html=True
        )
...
def test_secim_page(secilen_ders, secilen_konu):
    ...
    if konu_link:  # Link varsa göster
        st.markdown(
            f"<a href='{konu_link}' target='_blank'><button style='background-color:transparan; color: ; padding:6px; border:none; border-radius:8px; cursor:pointer;'>📕 Konu Notu</button></a>",
            unsafe_allow_html=True
        )
