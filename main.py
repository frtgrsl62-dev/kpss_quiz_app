...
# ===============================
# !!! KALDIRILDI: Aktif kullanıcı işlemleri (çoklu cihaz desteği için kaldırıldı)
# Bu fonksiyonlar küresel bir durum yaratarak oturumların karışmasına neden oluyordu.
# Artık Streamlit'in kendi session_state'ine güveneceğiz.
...
def kaydet_sonuclar_to_user(user, sonuclar):
    # Kullanıcılar dosyasını her zaman en güncel haliyle oku
    ...
    if giris_btn:
        ...
        if (k_adi in sabit_kullanicilar and sabit_kullanicilar[k_adi]["sifre"] == sifre) or \
           (k_adi in kullanicilar and kullanicilar[k_adi]["sifre"] == sifre):
            ...
            # --- DEĞİŞİKLİK ---
            # Artık dosya oluşturmuyoruz, sadece session_state'e kaydediyoruz.
...
def ders_secim_page():
    ...
    if st.button("🔻 Çıkış Yap 🔻"):
        # --- DEĞİŞİKLİK ---
        # Çıkış yaparken sonuçları kaydet ve session_state'i temizle.
        # Artık aktif kullanıcı dosyasını silmeye gerek yok.
        user = st.session_state.get("current_user")
...
# ===============================
# Router (Uygulama Yönlendirici)
# ===============================

# --- DEĞİŞİKLİK ---
# Uygulama başlangıcındaki karmaşık dosya kontrolü kaldırıldı.
# Artık sadece o anki oturumda 'current_user' var mı diye bakıyoruz.
# Her yeni tarayıcı/cihaz boş bir session_state ile başlar.
